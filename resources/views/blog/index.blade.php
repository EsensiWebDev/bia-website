            ...
            <div class="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 lg:mx-0 lg:max-w-none lg:grid-cols-3">
                @foreach ($articles as $article)
                    <article class="flex max-w flex-col items-start justify-between overflow-hidden group cursor-pointer">
                        <a href="{{ route('blog.show', [$article->category->slug, $article->slug]) }}" class="block w-full">
                        ...
                        <div class="flex items-center gap-x-2 text-sm uppercase my-3">
                            ...
                            <a href="{{ route('blog.category', $article->category->slug) }}" class="hover:text-[#203B6E]">
                                {{ $article->category->title }}
                            </a>
                        </div>
                        <div class="grow">
                            <a href="{{ route('blog.show', [$article->category->slug, $article->slug]) }}" class="block">
                                <h3
