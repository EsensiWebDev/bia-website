        ...
        <div class="mx-auto max-w-7xl lg:px-8 border-t border-gray-200 py-16 px-4">
            ...
            @else
                <div class="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 lg:mx-0 lg:max-w-none lg:grid-cols-3">
                    @foreach ($socialActivity as $social)
                        <article
                            class="flex max-w flex-col items-start justify-between overflow-hidden group cursor-pointer">
                            <a href="{{ route('social.show', $social->slug) }}" class="block w-full">
                                <div class="overflow-hidden w-full h-64">
                                    <img src="{{ asset('storage/' . $social->thumbnail) }}"
                                        alt="{{ $social->thumbnail_alt_text ?? 'Blog Article Bia Dental' }}"
                                        class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105">
                                </div>
                            </a>
                            <div class="grow mt-6">
                                <a href="{{ route('social.show', $social->slug) }}" class="block">
                                    <h3
                                ...
                                <p class="mt-2 line-clamp-3">
                                    {{ Str::limit(strip_tags($social->excerpt), 250) }} ...
                                </p>
                            </div>
                        </article>
                    @endforeach
            ...
            <div class="flex justify-center mt-10">
                {{ $socialActivity->onEachSide(1)->links('vendor.pagination.custom-tailwind') }}
            </div>
        </div>
    </section>
@endsection
